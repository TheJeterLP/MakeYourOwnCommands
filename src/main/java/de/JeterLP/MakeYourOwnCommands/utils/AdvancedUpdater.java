...
public class AdvancedUpdater {
    ...
    public void search() {
        ...
        if (versionCheck(this.versionName)) {
            MYOClogger.log(MYOClogger.Type.UPDATE, "A new update is available! (" + this.verwith + ") current: " + main.getDescription().getVersion());
    ...
    public void search(Player player) {
        ...
        if (versionCheck(this.versionName)) {
            player.sendMessage("§a[UPDATER]§7 A new update is available! (v" + this.verwith + ") current: " + main.getDescription().getVersion());
